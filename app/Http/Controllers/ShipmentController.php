...
class ShipmentController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $shipments = Shipment::query()
            ...
            ->when($user->isDispatcher(), fn($q) => $q->where('company_id', $user->company_id))
    ...
    public function store(StoreShipmentRequest $request): RedirectResponse
    {
        ...
        $shipment->customer->notify(new ShipmentCreated($shipment));
    ...
    public function destroy(Shipment $shipment): RedirectResponse
    {
        if (in_array($shipment->status, ['in_transit', 'out_for_delivery', 'picked_up', 'assigned'])) {
            return redirect()->back()
                ->with('error', "Cannot delete shipment {$shipment->tracking_number} while it is {$shipment->status}. Cancel the shipment first.");
        }
